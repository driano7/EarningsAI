  ...
  const commandsRes = await fetch(`https://api.telegram.org/bot${token}/setMyCommands`, {
    ...
    body: JSON.stringify({
      commands: [
        ...
        { command: "news", description: "Resumen diario de noticias (Supernota)" },
        { command: "superinvestors", description: "Movimientos 13F de superinversores" },
        { command: "portfolio", description: "Ver portafolio con P&L actual" },
