import { Text } from "./Text";
import { ComponentProps } from "react";
type AnimationState = "entering" | "visible" | "exiting";
interface FadingLettersFxProps extends ComponentProps<typeof Text> {
    text: string;
    animationState: AnimationState;
    messageIdx?: number;
}
declare function FadingLettersFx({ text, animationState, messageIdx, ...textProps }: FadingLettersFxProps): import("react/jsx-runtime").JSX.Element;
declare namespace FadingLettersFx {
    var displayName: string;
}
export { FadingLettersFx };
export type { FadingLettersFxProps, AnimationState };
//# sourceMappingURL=FadingLettersFx.d.ts.map